...
prefix="anope_"
die="yes"
ip2lockey="xxx5XWrC6wwhovxGRSdwOKmTkLoFerX2YCD8457vAPzmLg6QqOqwseVPgCA4eOe"
ipv6="false"

###########################

# The GeoIP data is created by MaxMind, available from www.maxmind.com.
geoip_country_source="https://www.ip2location.com/download/?token=${ip2lockey}&file=DB1LITECSV"
geoip_city_source="https://www.ip2location.com/download/?token=${ip2lockey}&file=DB3LITECSV"
...
download() {
    ...
    local desc=$3
    echo -n "   $desc     "
    wget -O $file --progress=dot $url 2>&1 | grep --line-buffered "%" | sed -u -e "s,\.,,g" | awk '{printf("\b\b\b\b%4s", $2)}'
    echo -ne " Done\n"
}
...
if test $geoip_database = "country"; then
	echo "Downloading..."
	download "$geoip_country_source" "GeoIPCountryCSV.zip" "Country Database:"
	echo "Unpacking..."
	unzip -jo GeoIPCountryCSV.zip
	rm GeoIPCountryCSV.zip
	echo "Converting to UTF-8..."
	iconv -f ISO-8859-1 -t UTF-8 IP2LOCATION-LITE-DB1.CSV -o $prefix"geoip_country.csv"
	rm IP2LOCATION-LITE-DB1.CSV
	echo "Importing..."
	mysqlimport --columns=start,end,countrycode,countryname $PARAMS $prefix"geoip_country.csv"
